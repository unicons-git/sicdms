...
public class EgovDoubleSubmitHelper {
	...
	public static boolean checkAndSaveToken(String tokenKey) {
		...
		if (session.getAttribute(EgovDoubleSubmitHelper.SESSION_TOKEN_KEY) == null) {
			throw new SicimsException("Double Submit Preventer TagLig isn't set. Check JSP.");
		}
		...
		// check parameter
		if (parameter == null) {
			throw new SicimsException("Double Submit Preventer parameter isn't set. Check JSP.");
		}
