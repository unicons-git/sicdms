...
Pager.prototype = {
	...
	createPaging : function(pageNumber){
		...
		if(pageNumber == this.selectIndex) { //현재 선택한 페이지
			...
			htmlStr += "<span style='cursor:pointer; border:1px solid #e3e3e3; display:inline-block; text-align:center; padding-top:6px; padding-bottom:6px; width:42px; margin-left:-1px; font-weight:bold;'>" + pageNumber + "</span></a>";
		} else {
			htmlStr = "<a id='a" + pageNumber + "' href='javascript:"+this.pagerObjectId+".goPage(" + pageNumber + ")'>";
			htmlStr += "<span style='cursor:pointer; border:1px solid #e3e3e3; display:inline-block; text-align:center; padding-top:6px; padding-bottom:6px; width:42px; margin-left:-1px;'>";
	...
	setBtnDisplay : function(){
		...
		if(groupNo == 1){ // 첫 번째 페이지 선택한 경우
			...
			$("#firstBtn" + this.num).attr("src", g3way.sicims.common.contextPath + "/resources/images/page/firstOn.gif");
			$("#prevBtn" + this.num).attr("src",  g3way.sicims.common.contextPath + "/resources/images/page/prevOn.gif");
		...
		if(groupNo == maxGroupNo){
			...
			$("#nextBtn" + this.num).attr("src", g3way.sicims.common.contextPath + "/resources/images/page/nextOn.gif");
			$("#lastBtn" + this.num).attr("src", g3way.sicims.common.contextPath + "/resources/images/page/lastOn.gif");
	...
	makePaging : function (index, totalCount, rows, totalPageObj, rangeCount) {
		...
		//전체 페이지 보다 index가 큰경우 1로 수정
		if(index > (totalCount / rows ) +1){
			index = 1;
		}
		...
		if (this.getItemTotal() > 0) {
			htmlStr+= "검색결과 : ";
			htmlStr+= "<span style=\"margin-top:-5px; font-weight:bold;color:#FF9900;\">" + this.getItemTotal() + "항목 (" + this.getSelectIndex() + "/" + this.getPageTotalCount() + ")<\/span>&nbsp;페이지";
			totalPageObj.show();
		} else {
			totalPageObj.hide();
		}
